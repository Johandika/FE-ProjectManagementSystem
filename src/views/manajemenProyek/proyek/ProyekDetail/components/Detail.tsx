...
import {
    ...
    Input,
    Notification,
    Switcher, // <-- Diubah dari Switch menjadi Switcher
    toast,
} from '@/components/ui'
// import { apiUpdateIsRetensi } from '@/services/ProyekService'
import { Field, FieldProps, Form, Formik } from 'formik'
import dayjs from 'dayjs'
import {
    apiCreateAdendumNilaiKontrak,
...
interface RetensiFormValues {
    is_retensi: boolean
    persen_retensi: number | string
}
...
const RetensiSchema = Yup.object().shape({
    is_retensi: Yup.boolean(),
    persen_retensi: Yup.number().when('is_retensi', {
        is: true,
        then: (schema) =>
            schema
                .min(1, 'Persentase harus lebih dari 0')
                .required('Persentase retensi wajib diisi'),
...
export default function Detail() {
    ...
    const location = useLocation()
    const [isSubmitting, setIsSubmitting] = useState(false)
    const [dialogRetensiOpen, setDialogRetensiOpen] = useState(false)
    const [dialogAdendumNilaiKontrakOpen, setDialogAdendumNilaiKontrakOpen] =
        useState(false)
    const [dialogAdendumTimelineOpen, setDialogAdendumTimelineOpen] =
        useState(false)

    const [retensiFormInitialValues, setRetensiFormInitialValues] =
        useState<RetensiFormValues>({
            is_retensi: false,
            persen_retensi: 0,
        })
    ...
    const handleOpenRetensiDialog = () => {
        setRetensiFormInitialValues({
            is_retensi: proyekData.is_retensi || false,
            persen_retensi: proyekData.persen_retensi || 0,
        })
        setDialogRetensiOpen(true)
    }

    const handleCloseRetensi = () => {
        setDialogRetensiOpen(false)
        setRetensiFormInitialValues({
            is_retensi: false,
            persen_retensi: 0,
        })
    ...
    const handleRetensiSubmit = async (
        values: RetensiFormValues,
        { setSubmitting }: { setSubmitting: SetSubmitting }
    ) => {
        setSubmitting(true)
        const projectId = proyekData.id
        ...
        const body = {
            id: proyekData.id,
            is_retensi: values.is_retensi,
            persen_retensi: values.is_retensi ? values.persen_retensi : 0,
        }
        // anchor2
        try {
            const result = await apiUpdateIsRetensi(body)
            console.log('result', result)
            if (result && result.statusCode === 200) {
                handleCloseRetensi()

                await dispatch(getProyek({ id: projectId }))

                popNotification('Retensi berhasil diperbarui')
            }
    ...
    const popNotification = (keyword: string) => {
        toast.push(
            <Notification title={`Berhasil`} type="success" duration={2500}>
                {keyword}
            </Notification>,
            {
                placement: 'top-center',
    ...
    const handleAdendumTimelineSubmit = async (
        ...
        try {
            const result = await apiCreateAdendumTimeline(processedData)

            if (result && result.data?.statusCode === 201) {
                dispatch(setUnreadNotification(true))
                popNotification('Data Adendum berhasil ditambahkan')
    ...
    const handleAdendumNilaiKontrakSubmit = async (
        ...
        try {
            const result = await apiCreateAdendumNilaiKontrak(processedData)
    ...
    return (
        <section>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2">
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 border-b py-4 items-start sm:items-center">
                            <div>
                                <div className="text-sm">Timeline :</div>
                                <div className="text-base font-semibold text-neutral-500">
                                    {formatDate(proyekData.timeline_awal || '-')} /{' '}
                                    {formatDate(proyekData.timeline_akhir || '-')}
                                </div>
                            </div>
                            {user.authority !== 'Owner' && (
                                <Button
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 border-b py-4 items-start sm:items-center">
                            ...
                            {user.authority === 'Super Admin' && (
                                <Button
                                    size="xs"
                                    variant="solid"
                                    onClick={handleOpenRetensiDialog}
                                >
                                    Ubah Retensi
                                </Button>
                            )}
            ...
            <Formik
                initialValues={retensiFormInitialValues}
                validationSchema={RetensiSchema}
                enableReinitialize={true}
                onSubmit={handleRetensiSubmit}
            >
                {({ errors, touched, isSubmitting, values }) => (
                    <Dialog
                        isOpen={dialogRetensiOpen}
                        onClose={handleCloseRetensi}
                        onRequestClose={handleCloseRetensi}
                    >
                        <h5 className="mb-4">Ubah Pengaturan Retensi</h5>
                        <Form>
                            <div className="flex flex-col gap-4">
                                <FormItem label="Aktifkan Retensi">
                                    {/* ## DIUBAH ## */}
                                    {/* Menggunakan Field agar terhubung otomatis ke Formik */}
                                    <Field
                                        name="is_retensi"
                                        component={Switcher}
                                    />
                                ...
                                {values.is_retensi && (
                                    <FormItem
                                        label="Persentase Retensi (%)"
                                        invalid={
                                            errors.persen_retensi &&
                                            touched.persen_retensi
                                        }
